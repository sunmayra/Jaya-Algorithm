...
//STANDARD 2013 CEC FUNCTION CODES for Jaya sequential

import java.util.Scanner;
import java.io.File;
public class JayaTestFunc13 {
	...
	void test_func(double[] x, double[] f, int nx, int mx,int func_num)throws Exception{
		...
		if (ini_flag==0) /* initiailization*/
		{
			...
			File fpt = new File("/home/sachin/Desktop/input_data"+"/M_D"+nx+".txt");//* Load M data *
			...
			fpt=new File("/home/sachin/Desktop/input_data"+"/shift_data.txt");
		...
		for (i = 0; i < mx; i++)
		{
			...
			case 2:	
				...
				break;	
			case 3:	
				f[i]=bent_cigar_func(t,f[i],nx,OShift,M,0);
				f[i]+=-1200.0;
				break;
			case 4:	
	...
	double ellips_func (double[] x, double f, int nx, double[] Os,double[] Mr,int r_flag) /* Rosenbrock's */
	{
	    int i;
		double tmp1,tmp2;
	...
	double bent_cigar_func (double[] x, double f, int nx, double[] Os,double[] Mr,int r_flag) /* Different Powers */
	{
		int i;
		shiftfunc(x, y, nx, Os);
		if (r_flag==1)
	...
	double escaffer6_func (double[] x, double f, int nx, double[] Os,double[] Mr,int r_flag) /* Expanded Scafferï¿½ï¿½s F6  */
	{
	    int i;
	    double temp1, temp2;
		shiftfunc(x, y, nx, Os);
